...
**Table names are prefixed `samnian_`** (`samnian_events`,
`samnian_event_interest`, `samnian_groups`, `samnian_group_members`) —
this app's Supabase project (`dwvjerviszikatdzuqkl`, dashboard-named
"projects") is shared with a completely unrelated app that already owns
unprefixed `events`/`event_interest` tables (a different schema entirely
— EOI/ticketing fields like `slug`, `season`, `price_pp`). Don't rename
these back to unprefixed without checking that project's table list first.

On that live project specifically, `drizzle/0000_first_mesmero.sql` and
`drizzle/0001_premium_morgan_stark.sql` were applied by hand (via direct
SQL, not `drizzle-kit migrate`) during the events/onboarding merge —
`0001` is really just the pre-existing `theme` column, which the
events-redesign branch's schema snapshot never had, showing up as a
"new" migration once merged. Running `npm run db:migrate` against that
