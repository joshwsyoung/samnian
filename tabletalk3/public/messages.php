...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <button class="btn d-md-none" data-bs-toggle="offcanvas" data-bs-target="#chatList">Chats</button>
        <button type="button" class="btn btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#conversationInfoModal">
    ...
    <div class="row">
        ...
        <div class="offcanvas offcanvas-start d-md-none" tabindex="-1" id="chatList">
                ...
                <?php else: ?>
                    <div class="list-group">
                        <?php foreach ($conversations as $c): ?>
                            ...
                            <div class="list-group-item <?= isset($_GET['conversation_id']) && $_GET['conversation_id'] == $c['id'] ? 'active' : '' ?>">
                                <div class="d-flex justify-content-between align-items-center">
                                    <a href="?conversation_id=<?= $c['id'] ?>" class="fw-bold text-decoration-none text-reset flex-grow-1">
                                ...
                                <?php if ($c['status'] === 'pending'): ?>
                                    <form action="<?= BASE_URL ?>/handlers/invite_decision_handler.php" method="post" class="d-flex gap-2">
                                        ...
                                        <button type="submit" name="decision" value="declined" class="btn btn-sm btn-danger">Decline</button>
                                    </form>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
        ...
        <div class="col-md-8">
                            ...
                            <div class="modal-body">
                                <?php foreach ($participants as $p): ?>
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <span class="badge bg-<?= $p['status'] === 'confirmed' ? 'success' : 'warning text-dark' ?>">
                                            <?= htmlspecialchars($p['name']) ?> (<?= $p['status'] ?>)
                                        </span>
                                        <?php if ($p['id'] != $conversation_owner_id): ?>
                                            <form method="POST" action="<?= BASE_URL ?>/handlers/remove_user_handler.php" class="ms-2">
                ...
                <div class="modal fade" id="deleteConfirmationModal" tabindex="-1" aria-labelledby="deleteConfirmationModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                                <form method="POST" action="<?= BASE_URL ?>/handlers/delete_conversation_handler.php" class="d-inline">
                                    <input type="hidden" name="conversation_id" value="<?= $conversation_id ?>">
                ...
                <?php else: ?>
                    <?php foreach ($messages as $msg): ?>
                        <div class="d-flex <?= $msg['sender_id'] == $user_id ? 'justify-content-end' : 'justify-content-start' ?>">
                            <div class="mb-2 px-3 py-2 rounded <?= $msg['sender_id'] == $user_id ? 'bg-primary text-white' : 'bg-white border' ?>" style="max-width: 75%;">
            ...
            <button class="btn btn-outline-success mb-3" data-bs-toggle="modal" data-bs-target="#inviteModal">+ Invite User</button>
